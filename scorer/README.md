...
Deployment
How to use the tool
Adding Supabase for team history
Design decisions
Extending the tool
Repository layout
...
Vercel setup
Import Content-TeamRG/tools on vercel.com
Set Root Directory to scorer
...
Go to the tool homepage
Ensure the URL tab is selected
Paste the full article URL (must be publicly accessible)
Click Score my content
The tool fetches the page, parses the HTML, runs all 29 checks, and returns the report in ~5 seconds
Text / draft mode

Switch to the Paste text tab
Paste your article in markdown format:
Use # Heading for H1
Use ## Heading for H2 sections
Click Score my content
Useful for scoring drafts before publishing
Reading the report
...
Bulk analyzer
Go to /bulk from the nav
Paste up to 20 URLs, one per line
Click Analyze all
Results appear ranked by score with section-level breakdown per URL on expand
Click Export CSV to download a spreadsheet of all scores — useful for reporting to stakeholders
Sharing a report
...
Open the SQL editor and run the contents of scorer/supabase-schema.sql:
...
Add both to .env.local locally and to Vercel Environment Variables in production
Redeploy
...
Decide which section it belongs to (A–F)
Open lib/scorer/section-[x].ts
Add a new exported function following this pattern:
...
Import and call it in lib/scorer/index.ts inside the scoreContent function
The report, dashboard, and all UI update automatically — no other changes needed
